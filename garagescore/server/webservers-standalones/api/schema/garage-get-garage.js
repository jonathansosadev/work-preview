...
const typePrefix = 'garageGetGarage';
...
module.exports.typeDef = gql`
  ...
  extend type Query {
    ${typePrefix}(garageId: ID!): ${typePrefix}Garage
  }
`;
...
module.exports.resolvers = {
  [`${typePrefix}Garage`]: {
    ...
    billingAccount: ({ _id }, _, { app }) =>
      app.models.BillingAccount.getMongoConnector().findOne({ garageIds: new ObjectID(_id) }),
    users: ({ _id }, _, { app }) =>
      app.models.User.getMongoConnector()
        .find({ garageIds: new ObjectID(_id) })
        .toArray(),
  },
  ...
  Query: {
    [typePrefix]: async (root, { garageId }, { app, scope: { logged, authenticationError, user } }) => {
      if (!logged) {
      ...
      return app.models.Garage.getMongoConnector().findOne({ _id: new ObjectID(garageId) });
    },
  },
};
