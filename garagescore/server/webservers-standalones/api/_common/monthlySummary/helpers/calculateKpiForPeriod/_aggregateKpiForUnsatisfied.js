...
/**
 * Performing an aggregate on datas to produce numbers displayed in Problem resolution tab
 ...
 */
module.exports = async (garages, period, dataType, hasGarageSubscribed, connector) => {
  ...
  const $project = {
    ...
    countUnsatisfied: getKpiExpr('countUnsatisfied'),
    countUnsatisfiedResolved: getKpiExpr('countUnsatisfiedClosedWithResolution'),
    countUnsatisfiedUnassigned: hasGarageSubscribed ? getKpiExpr('countUnsatisfiedAllUnassigned') : undefined,
    countUnsatisfiedNoAction: hasGarageSubscribed ? getKpiExpr('countUnsatisfiedAllAssignedWithoutAction') : undefined,
    countUnsatisfiedContacted: hasGarageSubscribed ? getKpiExpr('countUnsatisfiedAllAlreadyContacted') : undefined,
  };
   ...
   * Finally we'll compute countUnsatisfiedNoAction = countUnsatisfied - (countUnsatisfiedUnassigned + countUnsatisfiedContacted)
   */
  ...
  try {
    ...
    log.error(ANASS, `Fail : _aggregateKpiForUnsatisfied for period:  ${periodToken},  error : ${err}`);
    return [];
  }
};
