...
describe('Garage set subscriptions', () => {
  ...
  it('Should stop subscriptions', async () => {
    const mutation = gql`
      mutation garageSetStopSubscriptions($garageId: ID!, $billingAccountId: ID!) {
        garageSetStopSubscriptions(garageId: $garageId, billingAccountId: $billingAccountId) {
          active
          dateEnd
        }
      }
    ...
    const subscriptionsObject = data.garageSetStopSubscriptions;
    expect(subscriptionsObject.active).to.be.false;
    expect(subscriptionsObject.dateEnd).to.exist;
  });
});
