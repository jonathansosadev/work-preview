...
const {
  resolvers: {
    Mutation: { AutomationCampaignsAssignCustomContentToCampaigns },
  },
} = require('../../server/webservers-standalones/api/schema/automation-campaigns-assign-custom-content-to-campaigns');

const testApp = new TestApp();

describe('Apollo - Automation Campaigns Assign Custom Content', async function () {
  ...
  it('Add new custom content', async function () {
    ...
    let customContents = await testApp.models.AutomationCampaignsCustomContent.find({ where: {}, fields: {} });
    let campaigns = await testApp.models.AutomationCampaign.find({ where: {}, fields: {} });
    ...
    await AutomationCampaignsAssignCustomContentToCampaigns(
      {},
      {
        target: 'M_M',
        customContentId: campaigns.find((e) => e.displayName === 'El Taz').customContentId.toString(),
        garageIds: [garage2.id.toString()],
      },
      context
    );
    campaigns = await testApp.models.AutomationCampaign.find({ where: {}, fields: {} });
    expect(campaigns.find((e) => e.displayName === 'El Taz').customContentId.toString()).to.equals(
      customContents[0].getId().toString()
    );
    expect(campaigns.find((e) => e.displayName === 'El Taz Junior').customContentId).to.equals(undefined);
    expect(campaigns.find((e) => e.displayName === 'El Taz Senior').customContentId.toString()).to.equals(
      customContents[0].getId().toString()
    );
  });
});
