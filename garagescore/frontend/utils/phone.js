const phoneNumberSpecs = {
  ...
  NC: {
    ...
    validators: {
      ...
      mobile: (input) =>
        /^(5[0-4]|[79]\d|8[0-79])\d{4}$/.test(input.replace(/(\s|\.)/g, '')),
      landLine: (input) =>
        /^(2[03 -9]|3[0-5]|4[1-7])\d{4}$/.test(input.replace(/(\s|\.)/g, ''))
    },
    formatter(input) {
  ...
  BE: {
    ...
    validators: {
      ...
      landLine: (input) =>
        /^0?([1-356]\d{2}|4[0-5]\d|71\d|[89][1-9]\d|80[1-9])\d{5}$/.test(
          input.replace(/(\s|\.)/g, '')
        )
    },
    formatter(input) {
      ...
      if (cleanInput.length === 9)
        return cleanInput.replace(/(\d{3})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4')
  ...
  LU: {
    ...
    validators: {
      ...
      landLine: (input) =>
        /^(4|67|2[0-3589]|[3-57-9]\d|2[467](67|[2-57-9]\d))\d{4}$/.test(
          input.replace(/\s/g, '')
        )
    },
    formatter(input) {
      ...
      if (cleanInput.length === 5)
        return cleanInput.replace(/(\d{1})(\d{4})/, '$1 $2')
      if (cleanInput.length === 6)
        return cleanInput.replace(/(\d{2})(\d{2})(\d{2})/, '$1 $2 $3')
      if (cleanInput.length === 8)
        return cleanInput.replace(/(\d{2})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4')
      // Mobile
      if (cleanInput.length === 9)
        return cleanInput.replace(/(\d{3})(\d{3})(\d{3})/, '$1 $2 $3')
      return cleanInput
    }
  },
  ES: {
...
const _findSpec = (phoneNumber) => {
  const found = Object.keys(phoneNumberSpecs).find(
    (spec) => phoneNumber.indexOf(phoneNumberSpecs[spec].code) === 0
  )
 ...
 */
const validatePhoneNumberFromInput = (
  input,
  mode = 'any',
  forcedCountry = null
) => {
  ...
  if (mode !== 'any' && spec.validators[mode])
    return spec.validators[mode](cleanedInput)
  return Object.values(spec.validators).some((validator) =>
    validator(cleanedInput)
  )
}
...
const parsePhoneNumber = (input) => {
  ...
  let country = Object.keys(phoneNumberSpecs).find(
    (countryCode) =>
      cleanedInput.indexOf(phoneNumberSpecs[countryCode].code) === 0
  )
    ...
    country = Object.keys(phoneNumberSpecs).find((countryCode) => {
      ...
      return Object.keys(spec.validators).some(
        (type) =>
          ...
          (!spec.numberLength[type] ||
            cleanedInput.length === spec.numberLength[type])
      )
    })
  }
  ...
  if (!country) {
    country = Object.keys(phoneNumberSpecs).find((countryCode) =>
      [
        phoneNumberSpecs[countryCode].numberLength.mobile,
        phoneNumberSpecs[countryCode].numberLength.landLine
      ].includes(cleanedInput.length)
    )
  }
  if (!country) return null
  return {
    country,
    nationalPhoneNumber: cleanedInput.replace(
      phoneNumberSpecs[country].code,
      ''
    )
  }
}

export { phoneNumberSpecs, validatePhoneNumberFromInput, parsePhoneNumber }
