<template>
    <section class="contact-map"  v-show="this.mapboxApiToken !== '' && this.garage.longitude && this.garage.latitude">
      <div class="pull-right close-map-button" @click="closeMap()"><i class="icon-gs-close-circle"></i></div>
      <h4 class="map-title">
        {{ $t_locale('components/certificate/CtfContactMap')('itinerary', { name: garage.name, streetAddress: garage.streetAddress, postalCode: garage.postalCode, city: garage.city })}}
      </h4>
      <div id="map"></div>
    </section>
</template>
  ...
  export default {
    ...
    mounted: function () {
      if (!mapboxgl.supported()) {
        alert(this.$t_locale('components/certificate/CtfContactMap')('unsupportedMap'));
      }
    ...
    methods: {
      closeMap() {
        this.$store.commit('certificate/SET_MAP_VISIBLE', false);
      }
    }
  }
</script>
