...
export default {
  ...
  computed: {
    promotorsPercent() {
      return (this.row.countSurveyPromotor / this.row.countSurveyRespondedAll) * 100;
    },

    detractorsPercent() {
      return (this.row.countSurveyDetractor / this.row.countSurveyRespondedAll) * 100;
    },
    reviewRespondedPercentage() {
      return (this.row.countSurveysResponded / this.row.countReceivedAndScheduledSurveys) * 100;
    },
    certifUrl() {
  ...
  methods: {
    ...
    oneDecimal(value, isPercent) {
      ...
      return `${Number.parseFloat(Number.parseFloat(value).toFixed(1))}${isPercent ? '%' : ''}`;
    },
  },
};
</script>
