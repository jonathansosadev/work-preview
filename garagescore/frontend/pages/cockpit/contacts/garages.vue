...
import TableGarageContacts from '~/components/cockpit/contacts/garages/TableGarageContacts.vue';
...
export default {
  ...
  mixins: [chartsKpiMixin('COCKPIT_CONTACTS_GARAGES', KPI_BY_PERIOD_SINGLE_KPI, { kpiByPeriodSingle: {} })],
  ...
  data() {
    return {
      isSingleKpiByPeriodLoading: true,
      kpiByPeriodSingle: {},
  ...
  computed: {
    currentView() {
      ...
      return (componentName) => state[componentName]['view'];
    },
    chartData() {
      const chartData = this.$store.getters['cockpit/chart'];
      chartData.config.labels = chartData.config.labels.map(label => this.$t_locale('pages/cockpit/contacts/garages')(label.split('-')[1]) || label);
      return chartData;
    },
  },
  methods: {
