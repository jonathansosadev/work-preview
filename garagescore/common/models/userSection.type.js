 ...
 */
const urls = require('../lib/garagescore/urls');
...
module.exports = {
  ...
  getSectionFromName(name) {
    if (name.indexOf('cockpit-satisfaction') === 0) return 'COCKPIT_REVIEWS';
    if (name.indexOf('cockpit-contacts') === 0) return 'COCKPIT_CONTACTS';
    if (name.indexOf('cockpit-leads') === 0) return 'COCKPIT_LEAD';
    if (name.indexOf('cockpit-unsatisfied') === 0) return 'COCKPIT_UNSATISFIED';
    if (name.indexOf('cockpit-e-reputation') === 0) return 'COCKPIT_E-REPUTATION';
    if (name.indexOf('cockpit-analytics') === 0) return 'ANALYTICS';
    if (name.indexOf('cockpit-admin') === 0) return 'ADMIN';
    return null;
  },
};
