const translate = require('../../../../common/lib/garagescore/languages')('vehicle-bodytypes.js');
...
module.exports = {
  ...
  getBodyTypesByGarageType(garageType = GarageTypes.DEALERSHIP) {
    // Default
    return Object.values(bodyTypes[garageType] || bodyTypes[GarageTypes.DEALERSHIP]).map((t) => ({
      value: t,
      text: _displayName(t),
    }));
  },
  ...
  getFormattedString(data) {
    if (Array.isArray(data.get('lead.bodyType'))) return data.get('lead.bodyType').join(', ');
    return null;
  },
};
