...
const GarageHistoryPeriod = {
  ...
  getDisplayableDate(report) {
    ...
    if (report.reportConfigId === 'weekly' && report.minDate && report.maxDate) {
      return `du ${moment(report.minDate).format('DD MMMM YYYY')} au ${moment(report.maxDate).format('DD MMMM YYYY')}`;
    }
    if (!report.period) {
      return '*';
    }
    ...
    if (usedFormat && GarageHistoryPeriod[displayKey] && GarageHistoryPeriod[formatKey]) {
      return moment(period, GarageHistoryPeriod[formatKey]).format(GarageHistoryPeriod[displayKey]);
    }
    return period;
  },
  getFormatKeys() {
  ...
  async getCockpitAvailablePeriodsForUser(app, user, garageIds) {
    ...
    let garages = await getUserGarages(app, user.getId(), { dataImportStartedAt: true, createdAt: true }, $match);
    garages = garages.map((garage) => {
      // Can't clean this one, because importing garage-methods will introduce a cyclic require chain
      garage.getDataImportStartedAt = app.models.Garage.prototype.getDataImportStartedAt.bind(garage);
      return garage;
    });
    ...
    const oldestGarage = garages.reduce((garageA, garageB) =>
      moment(garageA.getDataImportStartedAt()).isBefore(garageB.getDataImportStartedAt()) ? garageA : garageB
    );

    return GarageHistoryPeriod.getCockpitAvailablePeriods(oldestGarage.getDataImportStartedAt());
  },
  getCockpitAvailablePeriods: function getCockpitAvailablePeriods(refDate, actualTime) {
    let now = actualTime;
    ...
    const allPeriods = [
      {
        ...
        selector: { from: formatPeriod(1, 'quarters_in_the_past'), to: formatPeriod('now') },
        groupId: 'others',
        minDate: GarageHistoryPeriod.getPeriodMinDate(GarageHistoryPeriod.LAST_QUARTER),
        maxDate: GarageHistoryPeriod.getPeriodMaxDate(GarageHistoryPeriod.LAST_QUARTER),
      },
      {
        id: GarageHistoryPeriod.CURRENT_YEAR,
        label: "de l'année en cours",
        display: 'Année en cours',
        selector: { from: formatPeriod(4, 'quarters_in_the_past'), to: formatPeriod('now') },
        groupId: 'others',
        minDate: GarageHistoryPeriod.getPeriodMinDate(GarageHistoryPeriod.CURRENT_YEAR),
        maxDate: GarageHistoryPeriod.getPeriodMaxDate(GarageHistoryPeriod.CURRENT_YEAR),
      },
      {
        ...
        groupId: 'others',
        minDate: GarageHistoryPeriod.getPeriodMinDate(GarageHistoryPeriod.ALL_HISTORY),
        maxDate: GarageHistoryPeriod.getPeriodMaxDate(GarageHistoryPeriod.ALL_HISTORY),
      },
    ];
    if (!refDate || moment(refDate).year() <= now.year() - 1) {
      ...
      allPeriods.push({
        ...
        selector: { from: formatPeriod(8, 'quarters_in_the_past'), to: formatPeriod(4, 'quarters_in_the_past') },
        groupId: 'others',
    ...
    for (let i = 1; i < 7; i++) {
      const month = moment(now).subtract(i, 'months').subtract(9, 'days');
      const periodDef = {
        id: month.format(GarageHistoryPeriod.MONTHLY_FORMAT),
        label: `du mois ${month.format(GarageHistoryPeriod.MONTHLY_DISPLAY)}`,
        display: _.capitalize(month.format(GarageHistoryPeriod.MONTHLY_DISPLAY)),
        selector: {
          from: formatPeriod(i, 'months_in_the_past', true),
          to: formatPeriod(i - 1, 'months_in_the_past', true),
        },
        groupId: 'monthly',
        minDate: GarageHistoryPeriod.getPeriodMinDate(month.format(GarageHistoryPeriod.MONTHLY_FORMAT)),
        maxDate: GarageHistoryPeriod.getPeriodMaxDate(month.format(GarageHistoryPeriod.MONTHLY_FORMAT)),
      };
      if ((refDate && moment(GarageHistoryPeriod.getPeriodMaxDate(periodDef.id)).isAfter(refDate)) || !refDate) {
        allPeriods.push(periodDef);
      }
    }
    for (let j = 1; j < 5; j++) {
      const quarter = moment(now).subtract(j, 'quarters').subtract(10, 'days');
      const periodDef2 = {
        id: quarter.format(GarageHistoryPeriod.QUARTER_FORMAT),
        label: `du trimestre ${quarter.format(GarageHistoryPeriod.QUARTER_DISPLAY)}`,
        display: `Trimestre ${quarter.format(GarageHistoryPeriod.QUARTER_DISPLAY)}`,
        selector: {
          from: formatPeriod(j, 'quarters_in_the_past', true),
          to: formatPeriod(j - 1, 'quarters_in_the_past', true),
        },
        groupId: 'others',
        minDate: GarageHistoryPeriod.getPeriodMinDate(quarter.format(GarageHistoryPeriod.QUARTER_FORMAT)),
        maxDate: GarageHistoryPeriod.getPeriodMaxDate(quarter.format(GarageHistoryPeriod.QUARTER_FORMAT)),
      };
      if ((refDate && moment(GarageHistoryPeriod.getPeriodMaxDate(periodDef2.id)).isAfter(refDate)) || !refDate) {
        allPeriods.push(periodDef2);
      }
    }
    return allPeriods;
  },
  getMustRegeneratePeriods() {
    return [
      GarageHistoryPeriod.ALL_HISTORY,
      GarageHistoryPeriod.CURRENT_YEAR,
      GarageHistoryPeriod.LAST_QUARTER,
      GarageHistoryPeriod.LEAD_ALL_HISTORY,
      GarageHistoryPeriod.UNSATISFIED_ALL_HISTORY,
    ];
  },
    ...
    switch (periodId) {
      ...
      default:
        ...
        if (periodId.match(GarageHistoryPeriod.DAILY_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.DAILY_FORMAT).add(1, 'day').subtract(1, 'millisecond').toDate();
        }
        if (periodId.match(GarageHistoryPeriod.WEEKLY_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.WEEKLY_FORMAT).add(7, 'day').subtract(1, 'millisecond').toDate();
        }
        ...
        if (periodId.match(GarageHistoryPeriod.YEARLY_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.YEARLY_FORMAT).add(1, 'year').subtract(1, 'millisecond').toDate();
        }
  ...
  getReferenceField(periodIdRaw) {
    ...
    periodId = periodId.toString(); // eslint-disable-line no-param-reassign
    if (periodId.match(GarageHistoryPeriod.DAILY_REGEX) || periodId.match(GarageHistoryPeriod.WEEKLY_REGEX)) {
  ...
  getPeriodMinDate(periodIdRaw) {
    ...
    switch (periodId) {
      ...
      case GarageHistoryPeriod.CURRENT_YEAR:
        return moment('00:00:00', 'hh:mm:ss').dayOfYear(1).toDate();
      case GarageHistoryPeriod.LAST_QUARTER:
      ...
      default:
        ...
        if (periodId.match(GarageHistoryPeriod.MONTHLY_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.MONTHLY_FORMAT).date(1).toDate();
        }
        if (periodId.match(GarageHistoryPeriod.QUARTER_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.QUARTER_FORMAT).toDate();
        }
        if (periodId.match(GarageHistoryPeriod.YEARLY_REGEX)) {
          return moment(periodId, GarageHistoryPeriod.YEARLY_FORMAT).month(0).date(1).toDate();
        }
        throw new Error(`Unsupported Period Id ${periodId}`);
    }
  },
  tokenizeLastMonth(referenceDate) {
    return moment(referenceDate).subtract(1, 'month').format(GarageHistoryPeriod.MONTHLY_FORMAT);
  },
  tokenizeLastWeek(referenceDate) {
    return moment(referenceDate).subtract(1, 'week').format(GarageHistoryPeriod.WEEKLY_FORMAT);
  },
  tokenizeLastday(referenceDate) {
    return moment(referenceDate).subtract(1, 'day').format(GarageHistoryPeriod.DAILY_FORMAT);
  },
  tokenizeLastQuarter(referenceDate) {
    return moment(referenceDate).subtract(1, 'quarter').format(GarageHistoryPeriod.QUARTER_FORMAT);
  },
  ...
  getLast12Months(refDate) {
    ...
    const getPastPeriod = (offset) => {
      return now.clone().subtract(offset, 'month').format(GarageHistoryPeriod.MONTHLY_FORMAT);
    };
  ...
  LAST_QUARTER_CONFIG: 'LAST_QUARTER',
};

module.exports = GarageHistoryPeriod;
