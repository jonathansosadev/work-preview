  ...
  getColumns({ i18n, fields }) {
    return [
      ...
      },
      {
        ...
        width: 20,
        resolve(data) {
          return data.get('leadTicket.customer.contact.mobilePhone') || '';
        },
      },
      {
        header: i18n.$t('Email'),
      ...
      {
        header: i18n.$t("Date data'identification du projet"),
        key: 'identifiedAt',
        width: 20,
        style: { numFmt: 'dd/mm/yyyy' },
        resolve(data, garage) {
        ...
        resolve(data) {
          if (data.get('type') !== DataTypes.MANUAL_LEAD || data.get('type') !== DataTypes.MANUAL_UNSATISFIED) {
            if (data.get('review.reply')) {
              ...
              // ? i18n.$dd(data.publicReviewCommentCreatedAt, 'short') : '';
            }
          }
          return '';
        },
      ...
      },
      {
        ...
        resolve(data, garage) {
          ...
          return [i18n.$t(data.get('source.type')), i18n.$t(data.get('source.by'))].filter((e) => e).join(' - ');
        },
      },
      {
        header: i18n.$t('Type de projet'),
        key: 'leadType',
        width: 20,
        resolve(data, garage) {
          if (!garage.isSubscribed('Lead')) {
      ...
      {
        header: i18n.$t('Type de véhicule'),
        key: 'leadSaleType',
        width: 20,
        resolve(data, garage) {
          if (!garage.isSubscribed('Lead')) {
      ...
      },
      {
        ...
        resolve(data, garage) {
          ...
          return data.get('lead.knowVehicle') || data.get('leadTicket.knowVehicle')
            ? data.get('lead.vehicle') || data.get('leadTicket.brandModel') || ''
            : i18n.$t('unknown');
        },
      },
      {
      ...
      {
        header: i18n.$t('Reprise souhaité'),
        key: 'leadTradeIn',
        width: 20,
        resolve(data, garage) {
          if (!garage.isSubscribed('Lead')) {
          ...
          return i18n.$t(data.get('leadTicket.tradeIn')) || '';
        },
      },
      {
      ...
      },
      {
        ...
        width: 20,
        resolve(data) {
          return data.get('leadTicket.status') === LeadTicketStatus.CLOSED_WITH_SALE
            ? i18n.$t('sold')
            : data.get('leadTicket.status') === LeadTicketStatus.CLOSED_WITHOUT_SALE
            ? i18n.$t('notSold')
            : '';
        },
      },
  ...
  async getRow({ i18n, fields, data: rawData, garage: rawGarage }) {
    ...
    await Promise.all(
      this.getColumns({ i18n, fields }).map(async (col) => {
        row[col.key] = col.resolve(data, garage, manager);
      })
    );
