...
module.exports = {
  ...
  pipeline: _automationEventPipeline(period, AutomationCampaignsEventsType.OPENED),
  ...
  message: async (res) => {
    const { countMobile, countDesktop } = (res && res.length > 0 && res[0]) || { countMobile: 0, countDesktop: 0 };
    return `Nombre anormal d'ouvertures Automation sur les 3 derniers jours (Mobile: ${countMobile}, Email: ${countDesktop})`;
  },
