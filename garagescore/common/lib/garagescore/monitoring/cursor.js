...
var fs = require('fs');
var os = require('os');
var JOURNAL_PATH = path.join(os.tmpdir(), 'mongodb-core.journal');
var JOURNAL_FILE = fs.createWriteStream(JOURNAL_PATH, { flags: 'w' });
console.log(`DEPRECATED node_modules/mongodb-core/lib/cursor.js:15 Mongodb core journal activated: ${JOURNAL_PATH}`);
// write in our journal
function journal(cmd, id, message) {
  const timestamp = Date.now();
  JOURNAL_FILE.write(`${[cmd, id, timestamp, message].join('|')}|\n`);
}
/**
...
var Cursor = function (bson, ns, cmd, options, topology, topologyOptions) {
  options = options || {};
  this._monitoring = { _id: Math.round(Math.random() * 100000000) };
  journal('NEW', this._monitoring._id, JSON.stringify(cmd));
  // Cursor pool
  this.pool = null;
  // Cursor server
  this.server = null;
...
Cursor.hasJournal = true;
//
// Handle callback (including any exceptions thrown)
var handleCallback = function (self, callback, err, result) {
  journal('CB', self._monitoring._id, result ? JSON.stringify(result).length : 0);
  try {
    callback(err, result);
  } catch (err) {
...
Cursor.prototype._find = function (callback) {
  ...
  if (self.logger.isDebug()) {
    self.logger.debug(
      f('issue initial query [%s] with flags [%s]', JSON.stringify(self.cmd), JSON.stringify(self.query))
    );
  }
...
var isCursorDeadButNotkilled = function (self, callback) {
  ...
  if (self.cursorState.dead && !self.cursorState.killed) {
    ...
    self.cursorState.documents = [];
    self.cursorState.cursorIndex = 0;
    handleCallback(self, callback, null, null);
    return true;
  }
...
var isCursorDeadAndKilled = function (self, callback) {
  if (self.cursorState.dead && self.cursorState.killed) {
    handleCallback(self, callback, MongoError.create('cursor is dead'));
    return true;
  }
...
var nextFunction = function (self, callback) {
  ...
  if (self.cursorState.cursorId == null) {
    ...
    self._find(function (err) {
      if (err) return handleCallback(self, callback, err, null);
    ...
    self._getmore(function (err, doc, connection) {
      if (err) return handleCallback(self, callback, err);
    ...
    if (!doc || doc.$err) {
      ...
      return setCursorDeadAndNotified(self, function () {
        handleCallback(self, callback, new MongoError(doc ? doc.$err : undefined));
      });
    }
    ...
    // Return the document
    handleCallback(self, callback, null, doc);
  }
};
