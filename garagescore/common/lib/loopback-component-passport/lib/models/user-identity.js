...
module.exports = function (UserIdentity) {
  ...
  function profileToUser(provider, profile, options) {
    ...
    if (!email && !options.emailOptional) {
      // Fake an e-mail
      email = (profile.username || profile.id) + '@loopback.' + (profile.provider || provider) + '.com';
    }
      ...
      function (err, identity) {
        ...
        if (identity) {
          ...
          return identity.updateAttributes(
            { profile: profile, credentials: credentials, modified: new Date() },
            function (err, i) {
        ...
        var query;
        // I won't change here because it's about login from 3rd party services and I don't think we ever use it
        if (userObj.email) {
          query = { or: [{ username: userObj.username }, { email: userObj.email }] };
        } else {
