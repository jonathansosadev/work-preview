...
import hostifyLogoIcon from '../../../assets/hostify-logo.png';
...
function PmsHostifyOriginRegister() {
  ...
  const registerHostifyUser = async (payload = {}) => {
    payload = getRegisterPayload(payload);
    const {data, error} = await api.auth.integrationRegister('hostify', payload);
    if (data) {
      mixpanelTrackWithUTM('Onboarding - Hostify');
  ...
  const onSubmit = async (data = {}) => {
    setIsLoading(true);
    await registerHostifyUser(data);
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <Content>
            <SyncTile>
              <Logo src={hostifyLogoIcon} alt="Hostify" />
...
export {PmsHostifyOriginRegister};
