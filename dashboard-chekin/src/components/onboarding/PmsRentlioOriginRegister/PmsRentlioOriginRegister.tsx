...
import rentlioLogoIcon from '../../../assets/rentlio-logo.svg';
...
function PmsRentlioOriginRegister() {
  ...
  const registerRentlioUser = async (payload = {}) => {
    payload = getRegisterPayload(payload);
    const {data, error} = await api.auth.integrationRegister('rentlio', payload);
    if (data) {
      mixpanelTrackWithUTM('Onboarding - Rentlio');
      setShowAccountCreatedModal(true);
    } else {
      if (
        error instanceof Object &&
        'api_token' in error &&
        error.api_token[0].startsWith('Please, tell')
      ) {
        setCollaboratorError(error.api_token[0]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    await registerRentlioUser(payload);
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <Content>
            <SyncTile>
              <Logo src={rentlioLogoIcon} alt="Rentlio" />
              <Form onSubmit={handleSubmit(onSubmit)}>
                <FormItemWrapper>
                  <Input
                    ...
                    disabled={isLoading}
                    label={t('api_token')}
                    error={errors[FORM_NAMES.api_token]?.message}
                    {...register(FORM_NAMES.api_token, {
...
export {PmsRentlioOriginRegister};
