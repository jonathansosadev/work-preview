...
import myvrLogoIcon from '../../../assets/myvr-logo.png';
...
const MYVR_CLIENT_ID = process.env.REACT_APP_MYVR_CLIENT_ID;
const MYVR_REDIRECT_URL = process.env.REACT_APP_MYVR_REDIRECT_URL;
...
function PmsMyvrOriginRegister() {
  ...
  const registerMyvrUser = React.useCallback(
    async (payload = {}) => {
      payload = getRegisterPayload(payload);
      const {data, error} = await api.auth.integrationRegister('myvr', payload);
      if (data) {
        mixpanelTrackWithUTM('Onboarding - MyVR');
  ...
  React.useEffect(() => {
    async function checkUrl() {
      ...
      const myvr_state = url.searchParams.get('state');
      if (myvr_state && monthlyPlanStatus === 'success') {
        const email = myvr_state.split(',')[0];
        const subscription_type = myvr_state.split(',')[1];
      ...
      } else if (!accommodationType && !myvr_state) {
        goBack();
      }
    }
  ...
  const redirectToMyvrOauth = (payload: any) => {
    const myvrOauthUrl = `https://developer.myvr.com/oauth/auth/?response_type=code&client_id=${MYVR_CLIENT_ID}&scope=property_read_write,reservation_read_write&redirect_uri=${MYVR_REDIRECT_URL}&state=${payload.email},${location.state.accommodationType}`;
    window.location.replace(myvrOauthUrl);
  };
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (isReadyToRegister) {
      ...
      if (myvr_state) {
        ...
        const payload = {
          email: data.email,
          subscription_type: myvr_state.split(',')[1],
          password: data.password,
          code: code,
        };
        await registerMyvrUser(payload);
      } else {
        setIsLoading(false);
      }
    } else {
      await redirectToMyvrOauth(data);
    }
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <RegisterSyncHeader syncOptions={accountSyncOptions}>MyVR</RegisterSyncHeader>
          <Content>
            <SyncTile>
              <Logo src={myvrLogoIcon} alt="MyVR" />
