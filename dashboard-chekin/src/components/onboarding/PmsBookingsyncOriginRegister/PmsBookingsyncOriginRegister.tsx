...
import bookingsyncLogoIcon from '../../../assets/bookingsync-logo.svg';
...
const BOOKINGSYNC_REDIRECT_URL = process.env.REACT_APP_BOOKINGSYNC_REDIRECT_URL;
...
function PmsBookingsyncOriginRegister() {
  ...
  const registerBookingsyncUser = React.useCallback(
    async (payload = {}, isNeedToSendSubscription) => {
      if (isNeedToSendSubscription) {
        payload = getRegisterPayload(payload);
      }
      const {data, error} = await api.auth.integrationRegister('bookingsync', payload);
      if (data) {
        mixpanelTrackWithUTM('Onboarding - Bookingsync');
  ...
  React.useEffect(() => {
    async function checkUrl() {
      ...
      const bookingsync_state = url.searchParams.get('state');
      if (bookingsync_state) {
        const areNeedRefetch = !!parseInt(bookingsync_state.split(',')[2]);
        if (areNeedRefetch) {
          if (!isRefetched) {
            setIsRefetched(true);
            setIsLoading(true);
            const payload = {
              ...
              access_token: code,
              email: bookingsync_state.split(',')[0].replace(' ', '+'),
            };
            registerBookingsyncUser(payload, false);
          } else {
            return;
          }
        }
      }
      if (bookingsync_state && monthlyPlanStatus === 'success') {
        const email = bookingsync_state.split(',')[0];
        const subscription_type = bookingsync_state.split(',')[1];
      ...
      } else if (!accommodationType && !bookingsync_state) {
        goBack();
      }
    }
  ...
  const redirectToBookingSyncOauth = (payload: any) => {
    const bookingsyncOauthUrl = `https://www.bookingsync.com/oauth/authorize?client_id=${BOOKINGSYNC_CLIENT_ID}&redirect_uri=${BOOKINGSYNC_REDIRECT_URL}&response_type=code&scope=bookings_read%20bookings_write%20clients_read%20clients_write%20rentals_read&state=${payload.email},${location.state.accommodationType},0`;
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (isReadyToRegister) {
      ...
      if (bookingsync_state) {
        ...
        const payload = {
          email: data.email,
          subscription_type: bookingsync_state.split(',')[1],
          password: data.password,
          access_token: code,
        };
        await registerBookingsyncUser(payload, true);
      } else {
        setIsLoading(false);
      }
    } else {
      await redirectToBookingSyncOauth(data);
    }
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <Content>
            <SyncTile>
              <Logo src={bookingsyncLogoIcon} alt="BookingSync" />
                ...
                <ButtonAndLoaderWrapper>
                  {isLoading ? (
                    ...
                    <Button
                      ...
                      light
                      size="big"
                      label={t('connect_with_bookingsync')}
                    />
                  )}
                </ButtonAndLoaderWrapper>
              </Form>
