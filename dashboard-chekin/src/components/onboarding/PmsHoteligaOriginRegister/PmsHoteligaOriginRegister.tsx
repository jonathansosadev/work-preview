...
import hoteligaLogoIcon from '../../../assets/hoteliga-logo.png';
...
type FormData = {
  ...
  [FORM_NAMES.external_username]: string;
  [FORM_NAMES.external_password]: string;
  [FORM_NAMES.external_domain]: string;
...
function PmsHoteligaOriginRegister() {
  ...
  const {
    ...
    ({queryKey}) => fetchPlan(queryKey[0], queryKey[1]),
    {
      ...QUERY_CONFIG,
    },
  );
  ...
  const registerHoteligaUser = async (payload = {}) => {
    payload = getRegisterPayload(payload);
    const {data, error} = await api.auth.integrationRegister('hoteliga', payload);
    if (data) {
      mixpanelTrackWithUTM('Onboarding - Hoteliga');
      setShowAccountCreatedModal(true);
    } else {
      if (error instanceof Object && 'external_username' in error) {
        setCollaboratorError(error.external_username[0]);
  ...
  const onSubmit = async (data = {}) => {
    setIsLoading(true);
    await registerHoteligaUser(data);
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <Content>
            <SyncTile>
              <Logo src={hoteligaLogoIcon} alt="Hoteliga" />
              <Form onSubmit={handleSubmit(onSubmit)}>
                <FormItemWrapper>
                  <Input
                    placeholder={t('enter_text') as string}
                    disabled={isLoading}
                    label={t('external_username')}
                    error={errors[FORM_NAMES.external_username]?.message}
                    {...register(FORM_NAMES.external_username, {
                ...
                <FormItemWrapper>
                  <Input
                    placeholder={t('enter_text') as string}
                    disabled={isLoading}
                    label={t('external_password')}
                    error={errors[FORM_NAMES.external_password]?.message}
                    {...register(FORM_NAMES.external_password, {
                      required: t('required') as string,
                    })}
                  />
                </FormItemWrapper>
                <FormItemWrapper>
                  <Input
                    ...
                    error={errors[FORM_NAMES.external_domain]?.message}
                    {...register(FORM_NAMES.external_domain, {
                      required: t('required') as string,
                    })}
                  />
                </FormItemWrapper>
...
export {PmsHoteligaOriginRegister};
