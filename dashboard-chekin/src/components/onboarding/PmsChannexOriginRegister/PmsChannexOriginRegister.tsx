...
import channexLogoIcon from '../../../assets/channex-logo.png';
...
type FormData = {
  [FORM_NAMES.core_email]: string;
  [FORM_NAMES.password]: string;
  [FORM_NAMES.repeatedPassword]: string;
  [FORM_NAMES.channex_email]: string;
  [FORM_NAMES.channex_password]: string;
};
...
function PmsChannexOriginRegister() {
  ...
  const email = watch(FORM_NAMES.core_email);
  ...
  const registerChannexUser = async (payload = {}) => {
    payload = getRegisterPayload(payload);
    const {data, error} = await api.auth.integrationRegister('channex', payload);
    if (data) {
      mixpanelTrackWithUTM('Onboarding - Channex');
  ...
  const onSubmit = async (data = {}) => {
    setIsLoading(true);
    await registerChannexUser(data);
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <Content>
            <SyncTile>
              <Logo src={channexLogoIcon} alt="Channex" />
              <Form onSubmit={handleSubmit(onSubmit)}>
                <FormItemWrapper>
                  <Input
                    placeholder={t('email') as string}
                    disabled={isLoading}
                    label={t('enter_your_channex_email')}
                    error={errors[FORM_NAMES.channex_email]?.message}
                    {...register(FORM_NAMES.channex_email, {
                ...
                <FormItemWrapper>
                  <Input
                    ...
                    disabled={isLoading}
                    label={t('enter_your_channex_password')}
                    type="password"
                    error={errors[FORM_NAMES.channex_password]?.message}
                    {...register(FORM_NAMES.channex_password, {
                      required: t('required') as string,
                    })}
                  />
                </FormItemWrapper>
                <FormItemWrapper>
                  <Input
                    ...
                    disabled={isLoading}
                    label={t('email')}
                    error={errors[FORM_NAMES.core_email]?.message}
                    {...register(FORM_NAMES.core_email, {
                      pattern: {
...
export {PmsChannexOriginRegister};
