...
import mewsLogoIcon from '../../../assets/mews_logo.svg';
...
type FormData = {
  ...
  [FORM_NAMES.access_token]: string;
...
function PmsMewsOriginRegister() {
  ...
  const registerMewsUser = async (payload = {}) => {
    payload = getRegisterPayload(payload);
    const {data, error} = await api.auth.integrationRegister('mews', payload);
    if (data) {
      mixpanelTrackWithUTM('Onboarding - Mews');
      setShowAccountCreatedModal(true);
    } else {
      if (error instanceof Object && 'access_token' in error) {
        setCollaboratorError(error.access_token[0]);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    await registerMewsUser(payload);
    setIsLoading(false);
  };
    ...
    <>
      ...
      <Wrapper>
        <DimensionsWrapper>
          ...
          <RegisterSyncHeader syncOptions={accountSyncOptions}>Mews</RegisterSyncHeader>
          <Content>
            <SyncTile>
              <Logo src={mewsLogoIcon} alt="Mews" />
              <Form onSubmit={handleSubmit(onSubmit)}>
                <FormItemWrapper>
                  <Input
                    placeholder={t('enter_your_access_token') as string}
                    disabled={isLoading}
                    label={t('access_token')}
                    error={errors[FORM_NAMES.access_token]?.message}
                    {...register(FORM_NAMES.access_token, {
